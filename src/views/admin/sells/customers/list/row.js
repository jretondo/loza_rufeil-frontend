...
const ProviderRow = ({
  ...
  const deleteUser = async (e, id, providerInfo, first, page) => {
    e.preventDefault();
    swal({
      title: 'Eliminar al cliente ' + providerInfo + '!',
      ...
      if (willDelete) {
        ...
        if (!response.error) {
          ...
          newActivity(`Se ha eliminado al cliente ${providerInfo})`);
          ...
          newAlert(
            'danger',
            'Hubo un error!',
            'Intentelo nuevamente. Error: ' + response.errorMsg,
          );
        }
      }
    });
  ...
  return (
    <tr key={id} className={loadingActions ? 'shimmer' : ''}>
      <td style={{ textAlign: 'center' }}>{provider.business_name}</td>
      <td style={{ textAlign: 'center' }}>{provider.document_number}</td>
      <td style={{ textAlign: 'center' }}>{ivaConditionStr}</td>
      <td className="text-right">
        <UncontrolledDropdown>
          ...
          <DropdownMenu className="dropdown-menu-arrow" right>
            ...
            <DropdownItem href="#pablo" onClick={(e) => details(e, provider)}>
            ...
            <DropdownItem
              ...
              onClick={(e) =>
                deleteUser(
                  e,
                  provider.id,
                  provider.business_name + ' CUIT: ' + provider.document_number,
                  first,
                  page,
                )
              }
