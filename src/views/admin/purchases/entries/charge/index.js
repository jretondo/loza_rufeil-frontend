...
const PurchasesEntriesCharge = ({
  ...
  const saveImportedInvoice = async () => {
    ...
    const response = await axiosPost(
      API_ROUTES.purchasesDir.sub.checkReceipts,
      data,
    );
    if (!response.error) {
  ...
  const getPaymentsMethods = async () => {
    const response = await axiosGetQuery(
      API_ROUTES.purchasesDir.sub.paymentsMethods,
      [],
    );
    if (!response.error) {
      const arrayData =
  ...
  const getProviderAccount = async () => {
    const response = await axiosGetQuery(
      API_ROUTES.providersDir.sub.parameters,
  ...
  const importedTaxes = (dataTaxes) => {
    ...
    const newArray = dataTaxes.map((item) => {
      const vat = vatRatesReceipts.find(
        (vat) => vat.vat_type_id === item.type && vat.vat_amount > 0,
      );
      console.log('vat :>> ', vat);
      if (vat) {
        return {
