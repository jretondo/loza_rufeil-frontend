...
const PurchasesEntriesCharge = ({
  ...
  const [paymentsMethodsRaw, setPaymentsMethodsRaw] = useState([]);
  ...
  const completeFieldsImported = () => {
    ...
    invoiceSelected.Customer && setSelectedProvider(invoiceSelected.Customer);
  };
  ...
  const saveNewReceipt = async () => {
    const data = {
      ...
      taxes: taxesList
        .filter((tax) => tax.amount > 0 && tax.active)
        .map((tax) => {
          return {
            ...tax,
            amount: roundNumber(tax.amount),
    ...
    const response = await axiosPost(API_ROUTES.sellsDir.sub.receipt, data);
    if (!response.error) {
      ...
      refreshListToggle();
      newActivity(
        'Se cargó un nuevo comprobante de ID: ' +
          response.data.id +
          ' con un total de $' +
          response.data.total +
          ' ',
  ...
  const getPaymentsMethods = async () => {
    ...
    if (!response.error) {
      ...
      setPaymentsMethodsRaw(arrayData);
    } else {
      setPaymentsMethods([]);
      newAlert(
        'danger',
      ...
      const arrayData =
        response.data.length > 0
          ? response.data.map((account, key) => {
              key === 0
                ? setDetail(account.description)
                : setDetail(`${detail}<br /> ${account.description}`);
              account.id = key;
              account.amount = 0;
  ...
  const getTaxes = async (vat_condition) => {
    ...
    if (!response.error) {
      ...
      newAlert(
        'danger',
        'Error al cargar los impuestos del proveedor',
        response.errorMsg,
      );
    }
  };
  ...
  useEffect(() => {
    parseInt(headerInvoice.sellPoint) > 0 &&
      setPaymentsMethods(
        paymentsMethodsRaw.filter(
          (item) => parseInt(headerInvoice.sellPoint) === parseInt(item.number),
        ),
      );
    // eslint-disable-next-line
  }, [headerInvoice.sellPoint, setPaymentsMethods]);
