...
const ProvidersListAccounts = ({
    ...
    const getAccounts = async () => {
        const response = await axiosGetQuery(API_ROUTES.providersDir.sub.parameters, [{ providerId: providerData.id }])
        if (!response.error) {
    ...
    const saveAccounts = async () => {
        ...
        const response = await axiosPost(API_ROUTES.providersDir.sub.parameters, {
            params: accountsToSave,
            providerId: providerData.id
        })
        if (!response.error) {
            newActivity(`Se guardaron las cuentas del proveedor ${providerData.business_name}`)
            newAlert("success", "Cuentas guardadas correctamente", "")
            refreshList()
            toggle()
        } else {
            newAlert("danger", "Error al guardar las cuentas al proveedor", response.errorMsg)
        }
    }
                        ...
                        return (<tr key={key}>
                            {
                                ...
                                <td className='text-center'>
                                    <InputSearch
                                        ...
                                        itemSelected={clientAccountsArray[key] ? clientAccountsArray[key].AccountChart : false}
                                        title={""}
                                        placeholderInput={"Busque una cuenta..."}
                                        getNameFn={(accountItem) => `${accountItem.name} (${accountItem.code})`}
                                        setItemSelected={(account) => {
                                            const newAccountsArray = clientAccountsArray.map((item) => {
                            ...
                            <td className='text-center' onDoubleClick={!clientAccount.change ? () => handleClick(key) : () => { }}>
                                {!clientAccount.change ? clientAccount.description :
                                    <Input
                                        type="textarea"
                            ...
                            <td>
                                <Button
                                    color="danger"
                                    onClick={() => {
                                        const newAccountsArray = clientAccountsArray.filter((account, index) => {
                                            return index !== key
                                        })
                                        setClientAccountsArray(newAccountsArray)
                                    }}
                    ...
                    <tr>
                        <td className='text-center'>
                            <Button
                                color="primary"
                                onClick={() => {
                                    setClientAccountsArray([...clientAccountsArray, {
                                        AccountChart: false,
                                        active: true,
                                        description: "",
                                        change: true,
